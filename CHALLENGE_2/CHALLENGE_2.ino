...
/////// PATH CONSTANTS ///////
float LEG_1_DISTANCE = 30.0; // in YARDS
float LEG_2_DISTANCE = 1.0;
float LEG_3_DISTANCE = 15.0;
float TIME_TO_FIRST_TURN = 30000; // 30s in millis
...
void loop(){
  ...
  if(UGV_WAS_TAGGED){
    ...
    if(totalDistTraveled < LEG_1_DISTANCE){
      ...
      if(currTime >= TIME_TO_FIRST_TURN){
        totalDistTraveled = LEG_1_DISTANCE;
        turn_right();
      }
      //goStraight(UGV_POWER, GYRO_TOL);
    }else if( (totalDistTraveled > LEG_1_DISTANCE) && (totalDistTraveled < LEG_1_DISTANCE+LEG_2_DISTANCE+1) ){
      ugv_forward(UGV_POWER);
      if(totalDistTraveled > LEG_1_DISTANCE+LEG_2_DISTANCE){
        totalDistTraveled = LEG_1_DISTANCE+LEG_2_DISTANCE+1;
        turn_left();
      }
    }else if( (totalDistTraveled > LEG_1_DISTANCE+LEG_2_DISTANCE+1) && (totalDistTraveled < LEG_3_DISTANCE) ){
      ugv_forward(UGV_POWER);
    }else{
      ugv_stop();
    }
